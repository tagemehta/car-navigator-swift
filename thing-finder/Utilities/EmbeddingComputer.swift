...
public enum EmbeddingComputer {
  ...
  public static func computeFeaturePrint(cgImage: CGImage) -> VNFeaturePrintObservation? {
  ...
  /// Computes an `Embedding` wrapper for an entire `CGImage`.
  ///
  /// - Parameter cgImage: The image to embed.
  /// - Returns: An `Embedding` on success; `nil` if Vision fails.
  public static func compute(cgImage: CGImage) -> Embedding? {
    guard let featurePrint = computeFeaturePrint(cgImage: cgImage) else { return nil }
  ...
  /// - Returns: The `Embedding` for the crop, or `nil` on failure.
