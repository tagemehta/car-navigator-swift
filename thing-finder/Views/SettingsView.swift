...
struct SettingsView: View {
  ...
  @AccessibilityFocusState private var metaGlassesFocused: Bool
      ...
      ScrollViewReader { proxy in
        List {
          ...
          Section(header: Text("Display Settings")) {
            Toggle("Show Favorites and Recents", isOn: $settings.showFavoritesAndRecents)
              .accessibilityHint("If you frequently get in the same car or paratransit vehicle")
            Text("Display your favorite and recent searches on the home screen.")
              .font(.caption)
              .foregroundColor(.secondary)
          }

          Section(header: Text("Advanced Settings")) {
            Toggle("Show Advanced Settings", isOn: $settings.showAdvancedSettings)
          }
          ...
          if settings.showAdvancedSettings {
            ...
            if FeatureFlags.metaGlassesEnabled {
              ...
              .id(SettingsAnchor.metaGlasses)
              .accessibilityFocused($metaGlassesFocused)
            }
        ...
        .onAppear {
          ...
          // If we were routed here before this view existed, onChange won't
          // fire, so honor a pending scroll request on appear too.
          if router.pendingScrollToMetaGlasses {
            scrollToMetaGlasses(proxy)
          }
        }
        ...
        // Deep-link target: reveal and scroll to the Meta Glasses section when
        // routed here (e.g. from the glasses connection overlay).
        .onChange(of: router.pendingScrollToMetaGlasses) { _, pending in
          if pending { scrollToMetaGlasses(proxy) }
        }
      }
    }
  }

  /// Reveal (advanced settings) and scroll/focus the Meta Glasses section.
  private func scrollToMetaGlasses(_ proxy: ScrollViewProxy) {
    // The section lives under Advanced Settings, so reveal it first.
    settings.showAdvancedSettings = true
    // Defer so the newly-revealed section exists before we scroll/focus.
    DispatchQueue.main.asyncAfter(deadline: .now() + 0.35) {
      withAnimation { proxy.scrollTo(SettingsAnchor.metaGlasses, anchor: .top) }
      metaGlassesFocused = true  // moves VoiceOver focus to the section
      router.pendingScrollToMetaGlasses = false
    }
  }
