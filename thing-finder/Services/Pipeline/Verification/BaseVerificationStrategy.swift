...
public class BaseVerificationStrategy: VerificationStrategy {
  ...
  /// - Returns: Publisher that emits verification outcome
  internal func performVerification(image: UIImage, candidate: Candidate) -> AnyPublisher<
    VerificationOutcome, Error
  > {
  ...
  internal func handleVerificationError(_ error: Error, for candidate: Candidate) -> AnyPublisher<
    VerificationOutcome, Error
  > {
  ...
  internal func meetsBasicRequirements(_ candidate: Candidate) -> Bool {
    ...
    let aspectRatio =
      candidate.lastBoundingBox.height / max(candidate.lastBoundingBox.width, 0.0001)
...
extension BaseVerificationStrategy {
  ...
  internal func calculatePriority(
    ...
    let attempts =
      strategyType == .trafficEye
      ? candidate.verificationTracker.trafficAttempts : candidate.verificationTracker.llmAttempts
