...
public class VerificationStrategyManager {
  ...
  public func verify(image: UIImage, candidate: Candidate, store: CandidateStore) -> AnyPublisher<
    (VerificationOutcome, String), Error
  > {
    guard let strategy = selectStrategy(for: candidate) else {
      DebugPublisher.shared.warning(
        "[VerificationStrategy][\(candidate.id.uuidString.suffix(8))] No suitable strategy found.")
      return Fail(error: VerificationError.noSuitableStrategy)
        .eraseToAnyPublisher()
    }
    DebugPublisher.shared.info(
      "[VerificationStrategy][\(candidate.id.uuidString.suffix(8))] Selected strategy: \(strategy.strategyName)"
    )
  ...
  /// - Returns: Publisher that emits verification outcome
  /// - Throws: `VerificationError.noSuitableStrategy` if no suitable strategy is found
  public func verify(image: UIImage, candidate: Candidate) -> AnyPublisher<
  ...
  > {
    ...
    return strategy.verify(image: image, candidate: candidate)
      .handleEvents(receiveOutput: { outcome in
        DebugPublisher.shared.info(
          "[VerificationStrategy][\(candidate.id.uuidString.suffix(8))] \(strategy.strategyName) result: match=\(outcome.isMatch)"
        )
      })
      .eraseToAnyPublisher()
  }
