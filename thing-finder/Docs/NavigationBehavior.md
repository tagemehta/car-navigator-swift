...
When haptics are enabled, it also emits one-shot success/failure haptics on match/reject transitions.
...
| Any         | `.lost` (angle ≤ 60°)                     | _No announcement_                                     |
| Any         | `.unknown`                                | _No announcement_                                     |
...
### Haptic Transition Feedback

When `settings.enableHaptics == true`, NavAnnouncer triggers haptics on status transitions:
...
| No target box                                          | _No announcement_                      |
| Speech disabled                                        | _No announcement_                      |
| Within cooldown                                        | _No announcement_                      |
...
When `settings.enableHaptics == true`, it also provides haptic pulses at the same interval as the beeps (slow pulses when off-center, fast pulses when centered).
...
### Haptic Pulsing Rules

1. **Start pulsing**: When target appears and `settings.enableHaptics == true`
2. **Update interval**: Smoothly adjust pulse rate based on centering (same interval mapping as beeps)
3. **Stop pulsing**: When target lost OR `settings.enableHaptics == false`
